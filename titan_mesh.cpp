#define _GNU_SOURCE // Guard: Must be at the absolute top for cpu_set_t and affinity macros
#include <iostream>
#include <vector>
#include <string>
#include <cstring>
...
// ==========================================
// 1. SYSTEM-LEVEL OPTIMIZATIONS (ANDROID SAFE)
// ==========================================
...
void lock_thread_to_realtime() {
    ...
    if (pthread_setschedparam(this_thread, SCHED_FIFO, &params) != 0) {
        // Safe fallback on unprivileged/non-rooted environments
    }
}
...
void lock_thread_to_core(int core_id) {
    ...
    // Bionic-compliant kernel thread pinning
    sched_setaffinity(0, sizeof(cpu_set_t), &cpuset);
}

// ==========================================
// 2. HARDWARE MATH KERNELS (ARM/NEON/X86 DUAL PROFILE)
// ==========================================
...
// ==========================================
// 4. ASTRA MESH NODE (NON-BLOCKING UDP)
// ==========================================
...
public:
    explicit TitanMeshNode(uint32_t id) : sock(-1), broadcast_addr{}, node_id(id) {
        ...
        // Enforce non-blocking processing matrix
        int flags = fcntl(sock, fcntl(sock, F_GETFL, 0));
    ...
    void listen_for_peers() {
        ...
        // Use ssize_t to explicitly match return layout and prevent -Wsign-compare errors
...
// ==========================================
// 5. SPATIAL CONTEXT & VISION PIPELINE
// ==========================================
...
int main() {
    std::cout << "🚀 Booting TitanUAI Sovereign Edge Node...\n";
    ...
    for (uint32_t frame = 0; frame < 200; ++frame) {
        ...
        // Check mesh non-blocking buffer for incoming swarm instructions
        ...
        usleep(10000); // 10ms hardware cycle timing
    }
